...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
        ...
        double res = presentValue;
        double drate = (100 + rate )/100;
        if (years == 0) {
            return res;
        }
        else {
            //for (int i = 1; i <= years; i++) {
             //   res *= drate;
            //}
            return Math.pow(drate, years) * res;
        }
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        double nomval = futureValue(presentValue, rate, targetYear);
        //Math.pow(2, 3)
        int years = numYears(targetYear);
        double drate = (100 - inflationRate )/100;
        return nomval * (Math.pow(drate, years));
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
        ...
        double nomval = totalSavings(perYear, targetYear, rate);
        //Math.pow(2, 3)
        int years = numYears(targetYear);
        double drate = (100 - inflationRate )/100;
        return nomval * (Math.pow(drate, years));

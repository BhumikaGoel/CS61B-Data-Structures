...
class Utils {
    ...
    /** Returns String and Stores FILE in /.gitlet/files/ . */
    static String storedFileName(File file) {
        String sha = sha1(readContents(file));
        return String.format("%s--%s", sha, file.getName());
    }

    /** Returns true if F1 and F2 have same contents.*/
    static boolean filesAreEqual(File f1, File f2) {
    ...
    /** Returns and Sorts a Collection C of type T.
     * Courtesy: StackOverFlow and Max Yao.*/
    static <T extends Comparable<? super T>> List<T> sortList(
            Collection<T> c) {
    ...
    /** Returns true if SHORTSTRING is the beginning substring of LONGSTRING. */
    static boolean isSubstring(String longString, String shortString) {
        return longString.substring(0, shortString.length()).equals(
                shortString);
    }

...
class CommandInterpreter {
    ...
    void loadStatement() {
        _input.next("load");
        String tableVal = this.name();

        Table table = Table.readTable(tableVal);
        _database.put(tableVal, table);
        _input.next(";");

        System.out.printf("Loaded %s.db%n", tableVal);
    }
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            tableNames.add(tableName());
            Table t1 = tableNames.get(0);
            Table t2 = tableNames.get(1);
            return t1.select(t2, columnNames, conditionClause(t1, t2));
        } else {
            Table t1 = tableNames.get(0);
            Table tnew = t1.select(columnNames, conditionClause(t1));
            return tnew;
        }
    ...
    Condition condition(Table... tables) {
        ...
        if (_input.peek().startsWith("'")) {
            colstr = literal();
            return new Condition(col1, rel, colstr);
        } else {

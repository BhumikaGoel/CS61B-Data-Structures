...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            char srcCol = move0.col0();
            char srcRow = move0.row0();
            char midCol = move1.col0();
            char midRow = move1.row0();
            Move newMove = move(srcCol, srcRow, midCol, midRow);
            return move(newMove, move1);
        }
        if (move0.jumpTail() == null) {
            char srcCol = move0.col0();
            char srcRow = move0.row0();
            char endCol = move0.col1();
            char endRow = move0.row1();
            return move(srcCol, srcRow, endCol, endRow, move1);
    ...
    private void toString(Formatter out) {
        ...
        out.format(""  + col0() + row0() + "-");
        if (jumpTail() == null) {
            out.format(""  + col1() + row1());
            return;
        }
        out.format(jumpTail().toString());
    }

...
class Model extends Observable {
    ...
    boolean tilt(Side side) {
        ...
        for (int c = 0;  c < _board.length; c++) {
            for (int r = 3; r > 0; r--) {
                if (vtile(c, r, side) == null) {
                    ...
                    if (vtile(c, newR, side) != null) {
                        setVtile(c, r, side, vtile(c, newR, side));
                        changed = true;
                    }
                }
            }
        ...
        for (int c = 0;  c < _board.length; c++) {
            for (int r = _board.length - 1; r > 0; r--) {
                Tile tile1 = vtile(c, r, side);
                Tile tile2 = vtile(c, r - 1, side);
                if ((tile1 != null) && (tile2 != null)) {
                    if (tile1.value() == tile2.value()) {
                        _score += 2 * vtile(c, r, side).value();
    ...
    private void checkGameOver() {
        for (int c = 0; c < _board.length; c++) {
            for (int r = 0; r < _board.length; r++) {
                if (_board[c][r] != null && _board[c][r].value() == MAX_PIECE) {
                    _gameOver = true;
                }
            }
        }
        ...
        if (!breaking && counter == size() * size()) {
            for (int c = 0; c < _board.length; c++) {
                for (int r = 0; r < _board.length; r++) {
                    if (c + 1 < size()) {
                        if (_board[c][r].value() == _board[c + 1][r].value()) {
                            _gameOver = false;
                            breaking = true;
                            break;
                        }
                    ...
                    if (r + 1 < size()) {
                        if (_board[c][r].value() == _board[c][r + 1].value()) {
                            _gameOver = false;
                            breaking = true;
                            break;
                        }
